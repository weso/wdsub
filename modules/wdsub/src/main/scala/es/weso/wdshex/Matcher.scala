...
/**
  ...
  */
case class Matcher(wShEx: WShEx, site: String = "http://www.wikidata.org/entity/", verbose: Boolean = false) {
  ...
  private def matchShapeExpr(shapeExpr: ShapeExpr, entity: Entity): MatchingStatus =
    shapeExpr match {
      ...
      case _ => {
        // TODO: Pending
        val notImplemented: es.weso.wdshex.MatchingError = NotImplemented(s"matchShape: $shapeExpr")
  ...
  private def matchPredicateValueSetValue(predicate: IRI, value: ValueSetValue, e: Entity, se: ShapeExpr) =
    value match {
